...
| `src/lib/accessibleTables.mjs`           | Build-time keyboard focus and accessible names for documentation tables.            |
...
After an approved master change, run `npm run brand:build`, review the film asset hashes in `videos/a2l-cinematic/src/brand-variant.json`, then rebuild the film and site and regenerate the sharing image below. `npm run brand:check` verifies that all exports, including the film assets, still match the master. The normal website build passes `--site-only` to verify its own exports without requiring the separate video folder, so `frontend/` can build on its own. Do not edit generated SVGs or PNGs independently.
...
The layout uses black, white, and layered zinc grays, with the logo's **#FFD54F** gold reserved for primary actions, the hero marker, and source indicators. Neutral text stays readable over faint gold citation backgrounds. Links use a quiet gold underline; small gold text and focus rings use darker `--gold-ink` in the light theme for contrast. Documentation navigation uses the same neutral selection in both sidebars and the mobile section menu: a soft gray fill, a fine outline, and stronger text. Navigation focus rings are neutral and distinct from selection. The shared tokens live in `shared.css`, with light and dark variants. The charcoal agent section keeps a consistent treatment in both themes.
...
File selection uses native radio inputs, prompt and mobile vault expansion use native disclosure controls, and the documentation uses Starlight's accessible search, tabs, navigation, and theme selector. Installation commands and fenced examples share Expressive Code's neutral panels and always-visible copy controls. Scrollable examples have distinct accessible names. Documentation tables receive keyboard focus and a name from their section heading during the Markdown build, so wide tables can be scrolled with arrow keys even without JavaScript. Page and agent-prompt copy failures show a local recovery message. Reduced-motion preferences are respected.

The documentation's section tracker selects the final section at the bottom of a page, even when that section is too short to reach the top of the viewport. A small correction in `src/scripts/docs-toc.ts` uses the pinned Starlight component's selection setter to keep the desktop outline and mobile summary synchronized. Starlight continues to handle normal scrolling. Re-run the TOC browser regression when upgrading Starlight.
...
The site uses **`release.published: true`** and advertises the verified PyPI version in `src/lib/site.ts`. The platform installer URLs and the agent's installation guide use the corresponding Git tag, so an unpublished version on `main` cannot break the live site's setup instructions. The `uv` command installs the latest published package.
...
The Vercel project is **`agent2learn`** under **`managementmos-projects`**, with production origin **https://agent2learn.vercel.app**. It is connected to **`ManagementMO/agent2learn`** on GitHub, with **`frontend`** as its Root Directory and files outside that directory excluded from the build. `vercel.json` selects Astro, installs with `npm ci`, builds with `npm run build`, and publishes only `dist/`. The project and package target Node.js 22.
...
In the same session, run `run-code --filename scripts/verify-site-browser.js` for the interaction and accessibility checks. It exercises citation focus, native keyboard file selection, mobile return and viewport changes, install/prompt/page copying, clipboard refusal, installation tabs, keyboard table scrolling, production search, and footer navigation. Its 58 scenarios include axe WCAG 2.2 AA checks over the homepage at five widths in both themes and all eleven guides at phone and desktop widths in both themes. Run in Chromium, Firefox, and WebKit. Both scripts use the origin of the open preview, so independent worktrees can use different ports.
...
Run `run-code --filename scripts/verify-toc-browser.js` in Chromium and WebKit to check all 11 guides in both themes at desktop and phone widths. It checks scrolling to the bottom, clicking the final section, opening its direct fragment URL, returning to earlier sections, synchronized mobile labels, viewport changes, and pages that fit without scrolling.
...
`check:release` builds a disposable copy with package availability switched on and a reserved test origin. It checks the published installation copy, agent prompt, documentation release notice, agent index, social image metadata, canonical URL, and sitemap, then removes the copy. On macOS/Linux it also executes the rendered installation command with a harmless download stub, proving that stdin reaches the installer and a failed download cannot execute partial output. It never runs the actual installer or onboarding. It leaves the real release setting and any running preview unchanged.
