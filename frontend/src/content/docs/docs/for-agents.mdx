...
Onboarding can install the skills into the vault's detected project directories for **Claude Code, Codex, Cursor, and Agent Skills-compatible tools**. To install or refresh global user-level skills later, run the standalone command:

```bash
a2l skills install
```

It targets detected global destinations by default, shows the destinations, and asks before writing. Use
`a2l skills install --project PATH` when you explicitly want project-local destinations. Local edits and
unrelated files are preserved; conflicts are reported.
