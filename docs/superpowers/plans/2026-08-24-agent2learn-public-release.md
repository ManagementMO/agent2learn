...
- [ ] **Step 2:** Write `pyproject.toml`. Standard runtime deps: `typer>=0.15,<1`,
...
- [ ] **Step 3:** Write `.gitattributes` with `* text=auto eol=lf` and `*.ps1 text eol=crlf`.
- [ ] **Step 4:** Write `.gitignore` covering `/agent2learn/` (root-anchored so it cannot hide
...
- [ ] **Step 4b:** Write `SECURITY.md` with private vulnerability reporting instructions and a
...
- [ ] **Step 4c:** Configure `detect-secrets` as a pre-commit check and document that it supplements,
...
- [ ] **Step 5:** Write `tests/test_smoke.py`:
      ...
      def test_version_is_importable():
          ...
          assert __version__
      ```
- [ ] **Step 6:** Run `uv run pytest -q`. Expected: 1 passed.
- [ ] **Step 7:** Commit.
      ```
...
- [ ] **Step 6:** `docs/install.md` — the same install content written **as instructions to an
      ...
      three options.
- [ ] **Step 7:** `docs/FUTURE.md` — record what was deliberately deferred and why: a Claude Code
