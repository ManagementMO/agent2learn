...
try{
  for(const variant of ['frontend','editorial']){
    ...
    assert.equal(geometry.height,variant==='frontend'?170:210);
    const path=variant==='frontend'?'assets/brand/frontend-mark.svg':'assets/brand/a2l-editorial-experiment.png';
    const src=await page.locator('#brand-mark').getAttribute('src');
    const data=`data:image/${variant==='frontend'?'svg+xml':'png'};base64,${(await readFile(path)).toString('base64')}`;
